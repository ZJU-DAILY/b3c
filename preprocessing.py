...
# def preprocess_graph(adj, layer=1, norm='sym', renorm=True):
...
def preprocess_graph(adjs,w=1.0):
    numView = len(adjs)
    adjs_normarlized = []
    for v in range(numView):
        adj = sp.coo_matrix(adjs[v])
        adj_ = adj + w*sp.eye(adj.shape[0])
        rowsum = np.array(adj_.sum(1))
        degree_mat_inv_sqrt = sp.diags(np.power(rowsum, -0.5).flatten())
...
def preprocess_graph_row(adjs,w=1.0):
    numView = len(adjs)
    adjs_normarlized = []
    for v in range(numView):
        adj = sp.coo_matrix(adjs[v])
        ...
        adj_normalized=degree_mat_inv_sqrt.dot(adj_).toarray()
        adjs_normarlized.append(adj_normalized.tolist())
    return np.array(adjs_normarlized)
